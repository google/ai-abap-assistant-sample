...
public section.
  ...
  class-data GT_CACHE_CONV type TYPE_T_CACHE_CONV .

  methods CONSTRUCTOR .
  methods FETCH_RESPONSE_VERTEXAI
    importing
    ...
    returning
      value(RS_RESPONSE) type ZIFCA_ABAP_ASSIST_AIUTIL=>TYPE_RESPONSE .
  methods GET_PROMPT
    importing
...
CLASS ZCLCA_ABAP_ASSIST_AIUTIL IMPLEMENTATION.
  ...
  METHOD call_llm.
    ...
    DATA:
      ...
      lt_cache           TYPE type_t_cache_conv,
    ...
    IF lo_aiclient IS NOT INITIAL.
      ...
      lo_aiclient->set_useragent_suffix(
                    ...
                    iv_module_identifier = /goog/cl_vertex_ai_sdk_utility=>c_useragent-gen_multimodal_ai_inv ) ).
  ...
  METHOD fetch_response_vertexai.
    DATA: lo_aiclient TYPE REF TO /goog/cl_aiplatform_v1.
    DATA:
      lv_p_projects_id   TYPE string,
      lv_p_locations_id  TYPE string,
  ...
  METHOD zifca_abap_assist_aiutil~execute_user_action.
    ...
    IF iv_prompt IS NOT INITIAL.
      IF iv_option = 'content' OR
         iv_option = 'eclipseRefactor'.
        fetch_response_vertexai(
          EXPORTING
            iv_prompt         = get_prompt(
                                  EXPORTING
                                    is_prompt_input  = VALUE #(
                                                        application_id = gc_llm_app_id
                                                        prompt_id =
                                                          SWITCH #(
                                                            iv_option
                                                              WHEN 'content'
                                                                THEN gc_llm_content_prompt_id
                                                              WHEN 'eclipseRefactor'
                                                                THEN gc_llm_content_refactor_id )
                                                        version = 1 )
                                    it_data_key = VALUE #(
                                      ( name = '{current_code_context}' value = iv_current_source )
                                      ( name = '{user_prompt}' value = iv_prompt ) ) )
            iv_option         = iv_option
